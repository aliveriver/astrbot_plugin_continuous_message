...
### 改进
- 🏷️ **元信息改用 XML 标签包裹**：引用消息与合并转发内容统一改为英文 XML 标签包裹，边界更清晰，降低 LLM 误解析风险
  - 引用消息：`[引用消息(...)]` → `<quoted_message sender="...">...</quoted_message>`
  - 合并转发内容：`【合并转发内容】\n...` → `<forward_content>\n...\n</forward_content>`
  - `forward_prefix` 配置默认值改为空字符串（标签本身已提供边界标识，无需额外前缀）
  - `bot_reply_hint` 配置项已移除：`sender` 属性（如 `sender="助手"`）已足以让 LLM 识别引用消息的来源，无需额外的系统提示标签
- 🔍 **更详细的调试日志**：在防抖核心流程（新建会话、追加消息、元信息合并）补充 `debug` 级别日志，便于排查问题
  - 合并后的完整消息日志由 `info` 降级为 `debug`，避免生产环境日志过多

### 兼容性
- ✅ 向后兼容：`reply_format` 为内部固定格式，无需用户配置；`forward_prefix` 仍可由用户自定义覆盖默认值
- ⚠️ `bot_reply_hint` 配置项已移除，若此前自定义过该项将不再生效
- ⚠️ 若曾自定义 `forward_prefix`，可按需手动改为 XML 格式以保持一致性，不强制要求
