...
| `enable_id_black_list` | bool | `false` | 启用 ID 黑名单。启用后名单内的用户消息不参与防抖合并，收到后立即处理（链接解析、图片本地化等功能仍生效）；名单为空时不过滤任何用户。 |
| `id_blacklist` | list | `[]` | 黑名单 ID 列表。填写用户 ID（如 QQ 号），也支持完整的会话来源标识。 |

黑名单优先级高于白名单：同一用户同时命中两者时，按黑名单处理。黑名单用户的消息不参与合并等待，收到后立即处理；链接解析、图片本地化等其余功能照常生效。白名单未命中的用户消息不会被插件拦截，按正常流程交给 LLM 或其他插件。
