...
**v2.5.0 更新**：
  ...
  引用消息和合并转发内容统一改用英文 XML 标签包裹（`<quoted_message sender="...">...</quoted_message>`、`<forward_content>...</forward_content>`），边界更清晰，降低 LLM 误解析风险；移除了冗余的 `bot_reply_hint` 系统提示配置（`sender` 属性已足够标明引用来源）；防抖核心流程补充了更详细的 `debug` 级别日志，便于排查问题。
...
- **类型**：字符串
- **默认值**：`""` （留空；合并转发内容现在由 `<forward_content>...</forward_content>` XML 标签自动包裹，无需额外前缀）
- **说明**：在提取的合并转发内容前添加的额外前缀文本（追加在 XML 包裹之前）
- **作用**：可留空，XML 标签已足够帮助 LLM 识别内容来源
