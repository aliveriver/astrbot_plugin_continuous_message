...
* 新增 `image_vision` 配置组，可为私聊图片指定独立的 VLM Provider。专用 VLM 识别成功后将图片描述交给主会话模型，避免 AstrBot 因主模型支持多模态而跳过配置的 VLM。
...
| 配置项 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `private_image_caption_provider_id` | string | `""` | 私聊图片专用识图模型的 Provider ID。配置后，插件先用该模型生成图片描述，再把描述交给主会话模型；主会话模型不会因此切换。留空则沿用 AstrBot 默认图片处理逻辑。 |
| `private_image_caption_prompt` | string | `请准确描述图片内容，并结合用户问题提取与问题相关的关键信息。` | 发送给专用识图模型的提示词。 |

专用识图模型在插件完成私聊消息合并和图片本地化后调用。识图成功时，原始图片不会再次作为附件发送给主会话模型，从而确保指定 VLM 的识别结果生效；识图失败时会保留原始图片并回退到 AstrBot 原有流程。
...
- 如果希望主会话模型和识图模型分工，填写 `image_vision.private_image_caption_provider_id`。Provider ID 需要使用 AstrBot 模型配置中的 ID，而不是模型名称。
